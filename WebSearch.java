...
public class WebSearch {
	...
	static Stack<SearchNode> VISITED;
		...
		while (st.hasMoreTokens())
		{
			...
			if (token.equalsIgnoreCase("<A"))
			{
				...
				{ // Collect the hypertext if this is a previously unvisited node.
					...
					SearchNode new_node = new SearchNode(hyperlink);
					
					do
					{
						token = st.nextToken();
						
						if(token.contains("QUERY1")){ multiplier++; heuristic++; } 
						else if(token.contains("QUERY2")){ multiplier++; heuristic++; }
						else if(token.contains("QUERY3")){ multiplier++; heuristic++; }
						else if(token.contains("QUERY4")){ multiplier++; heuristic++; }
						
						if (!token.equalsIgnoreCase("</A>")) { hypertext += " " + token; }
					}
					while (!token.equalsIgnoreCase("</A>"));
					
					new_node.setHvalue(heuristic * multiplier);
					parent.addNeighbor(new_node.getNodeName());
	 ...
	 * @param list
	 */
	static void sort(LinkedList<SearchNode> list){
...
class SearchNode
{
	...
	public int reportSolutionPath() {
		SearchNode cur = this;
		WebSearch.PATH.push(cur.nodeName);
		while(!WebSearch.VISITED.isEmpty()){
			SearchNode next = WebSearch.VISITED.pop();
			if(next.has_neighbor(cur)){
				WebSearch.PATH.push(next.getNodeName());
				cur = next;
			}
		}
		for(String node : WebSearch.PATH){
			System.out.print(node + " -> ");
		}
		//return the length - 1 so account for path length not number of nodes 
		return WebSearch.PATH.size() - 1;
	}
	...
	public boolean has_neighbor(SearchNode check){
